  ...
  github: async (user, account, profile, email, credentials) => {
    try {
      const response = await fetch(
        "http://backend:8000/api/dj-rest-auth/github/",
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok) {
        throw new Error(
          JSON.stringify({
            errors: "You are unable to sign up",
            status: false,
          }),
        );
      }
  ...
  providers: [
    CredentialsProvider.default({
      ...
      async authorize(credentials, req) {
        try {
          const response = await fetch(
            "http://backend:8000/api/dj-rest-auth/login/",
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
  ...
  callbacks: {
    ...
    async jwt({ user, token, account }) {
      ...
      if (getCurrentEpochTime() > token["ref"]) {
        const response = await fetch(
          "http://backend:8000/api/dj-rest-auth/token/refresh/",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
